...
# Aria2下载目录
downloadpath='/www/wwwroot/arovh.1315.ltd/download'

name1='googledrive'
name2='onedrive' #配置Rclone时填写的name
folder='/gv' #网盘里的文件夹，留空为整个网盘。
...
Task_INFO(){
    ...
    echo -e "\033[1;35mRemote path：\033[0m${remotepath1}"
    echo -e "\033[1;35mRemote path：\033[0m${remotepath2}"
    echo -e "-------------------------- [\033[1;33mINFO\033[0m] --------------------------"
    echo
}
...
Upload(){
    ...
    while [ $retry -le $retry_num -a -e "${uploadpath}" ]; do
        ...
        rclone copy -v "${uploadpath}" "${remotepath1}"
        rclone move -v "${uploadpath}" "${remotepath2}"
...
if [ "$path" = "$filepath" ] && [ $2 -eq 1 ] #普通单文件下载，移动文件到设定的网盘文件夹。
    then
        uploadpath=${filepath}
        remotepath1="${name1}:${folder}"
        remotepath2="${name2}:${folder}"
        Task_INFO
        Upload
        exit 0
    ...
    then
        uploadpath=${path}
        remotepath1="${name1}:${folder}/${rdp%%/*}"
        remotepath2="${name2}:${folder}/${rdp%%/*}"
        Task_INFO
        Upload
        exit 0
